...
public interface CrawlMapper extends BaseMapper<BkData> {
    ...
    @Select({"select t.qj jg_name,jh.jhrywl,jh.jhrsr,t.ryjl,t.rsr from(select qj,sum(ryjl)ryjl,sum(rsr)/1.06 rsr from(select ryjl,rsr,qj from lfq_crawl_data ,lfq_jg  where jg_name=tdz)group by qj)t,lfq_rjh jh where jh.qj=t.qj"})
    List<SendData> findAll();
